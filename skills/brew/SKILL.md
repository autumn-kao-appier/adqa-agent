---
name: brew
description: 蒸餾 skill — 從 Slack channel 和 Jira 萃取已結案的調查案例，轉成 schema.json 格式，寫入 __ADQA_AGENT_DIR__/knowledge/threads.jsonl（調查票）或 __ADQA_AGENT_DIR__/knowledge/tests.jsonl（實刷驗證票）。
---
...
| 檔案 | 用途 | index |
|---|---|---|
| `__ADQA_AGENT_DIR__/knowledge/threads.jsonl` | 調查票（root cause 分析、SSP 異常、pipeline bug） | `__ADQA_AGENT_DIR__/knowledge/threads_index.json` |
| `__ADQA_AGENT_DIR__/knowledge/tests.jsonl` | 實刷驗證票（summary 含 實刷/Validating/驗證/QA test） | `__ADQA_AGENT_DIR__/knowledge/tests_index.json` |

格式相同（schema.json），用 `provenance.source`、`status: by-design`、`root_cause.summary` 區分。
...
同時讀取兩個 index：
- `__ADQA_AGENT_DIR__/knowledge/threads_index.json`（調查票）
- `__ADQA_AGENT_DIR__/knowledge/tests_index.json`（實刷票）

各取：
- `last_distilled_at`：上次蒸餾時間（ISO8601 UTC）
- `processed.jira`：已處理的 Jira ticket key
- `processed.slack_threads`：已處理的 Slack thread_ts

---

## Step 1：掃描 Slack channel

依 `__ADQA_AGENT_DIR__/config/channels.yaml` 的 channel 清單，用 `slack_read_channel`（**response_format=detailed**，limit=50）逐頁讀取，直到訊息時間早於 `last_distilled_at`。
...
比對對應 index（`threads_index.json` 或 `tests_index.json`）的 `processed.jira`：
- 票號不存在 → 新案件，處理
...
Fallback 1 + 2 都找不到 root cause → 跳過，不寫進 threads.jsonl。不填猜測內容。
...
若符合 → 進入下方 **Test Case 蒸餾路徑**，跳過 Steps 4–9 的調查票流程。**寫入目標：`tests.jsonl` + `tests_index.json`**（不寫入 `threads.jsonl`）。
若不符合 → 繼續走原有流程（Steps 4–9），寫入 `threads.jsonl` + `threads_index.json`。

**去重前置檢查（Dedup Guard）：**
- 若票號已存在於 `tests_index.json.processed.jira` → 跳過，不重複處理
- 若票號已存在於 對應 index 的 `processed.jira` → 該票先前被誤歸為調查票，**遷移**：從 threads.jsonl 移除，改寫入 tests.jsonl（見 Step 12.1）
...
### TC-5：Upsert 到 tests.jsonl

同 Step 10 Atomic write 規則，但目標是 `tests.jsonl` + `tests_index.json`，不寫 `threads.jsonl`。
...
## Step 10：Upsert 到正確的 jsonl

**路由規則（在寫入前確認）：**
- Test ticket（Step 3.1 識別）→ `tests.jsonl` + `tests_index.json`
- 調查票 → `threads.jsonl` + `threads_index.json`
- 寫入 `threads.jsonl` 前：確認票號**不在** `tests_index.json.processed.jira`，否則跳過
...
**Atomic write（防止讀寫衝突）：**
```
1. 載入全部 threads.jsonl 到記憶體
2. 找到目標 case（透過 pattern_id 或 provenance.thread_refs）
3. 合併
4. 寫到 threads.jsonl.tmp
5. os.replace(threads.jsonl.tmp → threads.jsonl)
```
...
```json
{
  ...
  "total_cases": "{threads.jsonl 的總行數}",
  "processed": {
    "jira": {
      "ADQA-XXXX": "{處理時間}"
    },
...
def load_ticket_keys(jsonl_path):
    keys = {}
    with open(jsonl_path) as f:
        for line in f:
            case = json.loads(line)
            for k in case.get('volatile', {}).get('ticket_ids', []):
                keys[k] = case['pattern_id']
    return keys

cases_keys = load_ticket_keys('__ADQA_AGENT_DIR__/knowledge/threads.jsonl')
val_keys   = load_ticket_keys('__ADQA_AGENT_DIR__/knowledge/tests.jsonl')

overlaps = set(cases_keys) & set(val_keys)
if overlaps:
    print("⚠️ Dedup 警示：以下票號同時出現在兩個 DB：")
    for k in overlaps:
        print(f"  {k} → cases: {cases_keys[k]}, validations: {val_keys[k]}")
```

**發現 overlap 時的處理規則：**
1. 判斷票的性質（summary 有無 實刷/Validating/驗證/QA test）
2. 實刷票 → 從 `threads.jsonl` 移除，保留 `tests.jsonl`，更新兩個 index
3. 調查票 → 從 `tests.jsonl` 移除，保留 `threads.jsonl`，更新兩個 index
4. 不確定 → 在 brew 摘要中列出，讓人工判斷

**目前已知的潛在 overlap：**
- `ADQA-1802`（Rakuten parallel ping QA）在 `threads.jsonl` 但性質為驗證票，應考慮遷移至 `tests.jsonl`
